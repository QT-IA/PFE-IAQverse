...
(function (window) {
    ...
    function applyAlertPointsActivation(map, actionsMap) {
        ...
        allAlertPoints.forEach((el) => {
            ...
            // Pour les points auto-générés, garder toujours une severity appropriée
            const isAutoGenerated = el.getAttribute('data-auto-generated') === 'true';
            if (isAutoGenerated && severity === 'info') {
                // Les points auto-générés sans alerte restent "info" (verts dans le tableau)
                // Mais ils sont quand même affichés dans la 3D
            }
            ...
            // Ne jamais masquer les points auto-générés par three-scene.js
            // Ils sont gérés par updateAlertPoints() qui vérifie la visibilité 3D
            const isAutoGenerated = el.getAttribute('data-auto-generated') === 'true';
            
            if (isAutoGenerated) {
                // Pour les points auto-générés, forcer l'affichage et laisser three-scene.js gérer la visibilité
                el.style.display = 'block';
                el.setAttribute('data-active', 'true');
                console.log(`[alerts-engine] Forcing display for auto-generated point: ${key}`);
                activatedCount++;
                
                // Synchroniser immédiatement le tableau pour les points auto-générés
                try {
                    if (typeof window.syncAlertPointsToTable === "function")
                        window.syncAlertPointsToTable();
                } catch (e) { }
            } else {
                // Pour les points statiques: masquer si severity="info"
                if (severity === 'info') {
